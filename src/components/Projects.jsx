  ...
  if (thumb === 'shield') return (
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 p-6">
      <div className="flex items-center justify-center w-16 h-16 rounded-2xl"
        style={{ background: 'rgba(178,135,132,0.12)', border: '2px solid rgba(178,135,132,0.25)' }}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#b28784" className="w-8 h-8 opacity-80">
      ...
      <div className="w-full space-y-2 opacity-60">
        <div className="flex items-center gap-2">
          <div className="h-2 rounded-full flex-1" style={{ background: 'rgba(178,135,132,0.4)' }} />
          <div className="text-[10px] font-mono" style={{ color: '#b28784' }}>SSL ✓</div>
        </div>
        <div className="flex items-center gap-2">
          <div className="h-2 rounded-full flex-1" style={{ background: 'rgba(178,135,132,0.25)' }} />
          <div className="text-[10px] font-mono" style={{ color: '#b28784' }}>HTTPS ✓</div>
        </div>
        <div className="flex items-center gap-2">
          <div className="h-2 rounded-full" style={{ background: 'rgba(178,135,132,0.5)', width: '70%' }} />
          <div className="text-[10px] font-mono" style={{ color: '#b28784' }}>Score: 87</div>
        </div>
      </div>
    </div>
  )
  ...
  if (thumb === 'laundry') return (
    <div className="absolute inset-0 p-5 flex flex-col gap-3">
      <div className="flex gap-2 opacity-70">
        {['rgba(178,135,132,0.6)', 'rgba(224,204,203,0.7)', 'rgba(138,100,98,0.5)'].map((bg, i) => (
      ...
      <div className="flex-1 rounded-lg opacity-60" style={{ background: 'rgba(178,135,132,0.1)', border: '1px solid rgba(178,135,132,0.2)' }}>
        <div className="p-2 space-y-1.5">
          {[['#001', 'Pickup', 'rgba(178,135,132,0.5)'], ['#002', 'Washing', 'rgba(224,204,203,0.6)'], ['#003', 'Ready', 'rgba(138,100,98,0.4)']].map(([id, status, bg]) => (
            <div key={id} className="flex items-center justify-between">
              <span className="font-mono text-[10px]" style={{ color: '#9a7a78' }}>{id}</span>
  ...
  if (thumb === 'portfolio') return (
    <div className="absolute inset-0 flex items-center justify-center p-5">
      <div className="w-full rounded-xl overflow-hidden opacity-70"
        style={{ border: '1.5px solid rgba(178,135,132,0.3)', background: '#f9f8f6' }}>
        <div className="flex items-center gap-1.5 px-3 py-2" style={{ background: 'rgba(178,135,132,0.1)' }}>
          {['rgba(178,135,132,0.5)', 'rgba(224,204,203,0.6)', 'rgba(138,100,98,0.4)'].map((bg, i) => (
            <div key={i} className="w-2.5 h-2.5 rounded-full" style={{ background: bg }} />
          ))}
          <div className="flex-1 mx-2 h-3 rounded-full" style={{ background: 'rgba(178,135,132,0.15)' }} />
        </div>
        <div className="p-3 space-y-2">
          <div className="h-4 rounded w-1/2" style={{ background: 'rgba(178,135,132,0.3)' }} />
          <div className="h-2 rounded w-full" style={{ background: 'rgba(178,135,132,0.15)' }} />
          <div className="h-2 rounded w-5/6" style={{ background: 'rgba(178,135,132,0.15)' }} />
          <div className="flex gap-2 mt-2">
            <div className="h-6 w-20 rounded-lg" style={{ background: 'rgba(178,135,132,0.35)' }} />
            <div className="h-6 w-20 rounded-lg" style={{ background: 'rgba(178,135,132,0.15)', border: '1px solid rgba(178,135,132,0.3)' }} />
          </div>
        </div>
      </div>
    </div>
  ...
  if (thumb === 'todo') return (
    <div className="absolute inset-0 flex items-center justify-center p-5">
      <div className="grid grid-cols-3 gap-2 w-full opacity-70">
        {[
          { label: 'Now', items: 2, col: 'rgba(178,135,132,0.5)' },
          { label: 'Next', items: 3, col: 'rgba(224,204,203,0.6)' },
          { label: 'Later', items: 1, col: 'rgba(138,100,98,0.35)' },
        ].map(({ label, items, col }) => (
          <div key={label} className="space-y-1.5">
            <div className="text-[10px] font-mono font-medium mb-2" style={{ color: '#9a7a78' }}>{label}</div>
